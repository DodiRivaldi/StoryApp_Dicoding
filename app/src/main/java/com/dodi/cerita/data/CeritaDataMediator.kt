    ...
    override suspend fun load(
        ...
        val page = when(loadType){
            ...
            LoadType.PREPEND->{
                ...
                val prev =key?.prev?:return MediatorResult.Success(endOfPaginationReached = key!= null)
                prev
            }
            LoadType.APPEND->{
                val key = getKeyLastItem(state)
                val next = key?.next?:return MediatorResult.Success(endOfPaginationReached = key!=null)
                next
            }
        }
        ...
        try {
            ...
            database.withTransaction {
                ...
                response.listStory.forEach {
                    val cerita = CeritaItem(it.id,it.name,it.description,it.photoUrl,it.createdAt,it.lat,it.lon)
